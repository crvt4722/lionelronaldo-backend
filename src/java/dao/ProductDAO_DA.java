...
public class ProductDAO_DA {
     public static int getSoLuongSanPham(){
        try ( Connection c = openConnection()) {
            String select = "SELECT count(*) as tongSanPham FROM lionelronaldo.product;";
     ...
     public static ArrayList<String> getListAnh(int id_p){
        ...
        try (Connection c = openConnection()){
            ...
            String select = "SELECT * FROM lionelronaldo.product_image WHERE product_id = '%d'";
    ...
    public static List<Product> getTop5SanPham(){
        ...
        try(Connection c = openConnection()){
            String select = "SELECT p.*,sum(o.quantity) as tongdaban FROM lionelronaldo.product p, lionelronaldo.order o, lionelronaldo.warehouse w\n" +
