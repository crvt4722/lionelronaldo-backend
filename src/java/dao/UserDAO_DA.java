...
public class UserDAO_DA {
    public static int getSoLuongUser(){
        try ( Connection c = openConnection()) {
            String select = "SELECT count(*) as soUser FROM lionelronaldo.user_profile u where u.role = \"user\";";
        ...
        List<User_DA> list = new ArrayList<>();
         try ( Connection c = openConnection()) {
            String select = "SELECT u.*,sum(od.total_amount) as tongdaban " +
                            "FROM lionelronaldo.user_profile u, lionelronaldo.product p, lionelronaldo.order od, lionelronaldo.warehouse w\n" +
